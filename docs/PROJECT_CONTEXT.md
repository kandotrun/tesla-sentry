...
この境界には、R2作成、認証、課金、FFmpeg、CV、VLM、実映像の保存を含めない。

## 次の実装境界

1. Kanの実TeslaCamフォルダで命名、イベント集約、MP4事前検査を確認し、匿名化できるfixtureまたは期待値を固定する
2. 事前検査結果とアップロード可否の契約を定義する
3. ローカル開発用R2で直接multipart uploadと中断再開を実装する

実TeslaCamデータの互換性を確認する前に、本番R2、認証、課金へ進まない。
